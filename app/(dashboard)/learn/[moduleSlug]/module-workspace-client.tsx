...
function QuestionsTab({
  ...
  const passed = submitted && score >= Math.ceil(questions.length * 0.8);
  ...
  function handleSubmit() {
    ...
    setScore(correct);
    setSubmitted(true);
    if (correct >= Math.ceil(questions.length * 0.8)) onPass();
  }
  ...
  return (
    <div>
      <SectionLabel>{`${questions.length} Questions`}</SectionLabel>
...
export function ModuleWorkspaceClient({
  content,
}: {
  ...
  const objectives = meta
    ? meta.overview.objectives
    : [
  ...
  const keyConcepts = meta
    ? meta.overview.keyConcepts
    : ["Bitcoin UX", "Mental Models", "Design Constraints", "User Research", "Onboarding"];
  const resources: ResourceRow[] = meta ? meta.resources : mockResources;
  const sessions = meta ? meta.sessions : mockSessions;
  const videoUrl = meta ? meta.lesson.videoUrl : mockVideoUrl;
  const videoNote = content?.meta.lesson.videoNote;
  const readingMinutes = meta ? meta.lesson.readingMinutes : 12;
  const quizQuestions: QuizQuestion[] = content ? meta!.quiz : mockQuizQuestions;
  const assignment = meta?.assignment ?? null;
  const initialLessons = meta
    ? [{ number: "01", title: meta.title, status: "current" as const }]
    : mockLessons;

  const tabCounts: Partial<Record<Tab, number>> = content
    ? {
        Resources: resources.length,
        ...(assignment ? { Assignments: 1 } : {}),
        ...(quizQuestions.length > 0 ? { Questions: quizQuestions.length } : {}),
      }
    : {
        Resources: 4,
        Questions: 5,
  ...
  // Filter tabs: hide Assignments if no assignment, hide Questions if no quiz questions
  const tabs = allTabs.filter((t) => {
    if (t === "Assignments" && !assignment) return false;
    if (t === "Questions" && quizQuestions.length === 0) return false;
    return true;
  });
  ...
  const [lessonStatuses, setLessonStatuses] = useState(initialLessons);
    ...
    >
      ...
      >
        ...
        >
                  ...
                  {objectives.map((obj, i) => (
                    <li
                      ...
                      style={{
                        ...
                        marginBottom: i < objectives.length - 1 ? 14 : 0,
                      }}
                    >
              ...
              >
                ...
                >
                  ...
                  <p style={{ fontFamily: font.body, fontSize: "14.5px", lineHeight: "22px", fontWeight: 400, color: "var(--color-text-secondary)", margin: 0, marginTop: 8 }}>
                    A detailed written lesson with explanations, examples, Bitcoin concept breakdowns, and step-by-step walkthroughs. Estimated reading time: {readingMinutes} min.
                  </p>
          ...
          {activeTab === "Resources" && (
            <div>
              {mockResources.length > 0 ? (
                <>
                  <SectionLabel>{`${resources.length} Resources`}</SectionLabel>
                  <div style={{ marginTop: 20 }}>
                    {resources.map((r, i) => {
                      const row = (
                        <div
                          style={{
                            padding: "14px 16px",
                            borderBottom: i < resources.length - 1 ? "1px solid var(--color-border-subtle)" : "none",
                            display: "flex",
                            alignItems: "center",
                            gap: 12,
                            borderRadius: 10,
                      ...
                      return r.url ? (
                        <a
                          ...
                          style={{ textDecoration: "none", display: "block", outlineOffset: -2 }}
                        >
                          {row}
                        </a>
                      ) : (
          ...
          {/* ASSIGNMENTS TAB */}
          {activeTab === "Assignments" && assignment && content && (
            <div>
              ...
              >
                ...
                >
                  <div>
                    <h3 style={{ fontFamily: font.display, fontSize: 18, lineHeight: "24px", fontWeight: 600, color: "var(--color-text-primary)", margin: 0 }}>
                      {assignment.title}
                    </h3>
                    <p style={{ fontFamily: font.mono, fontSize: 11, lineHeight: "14px", fontWeight: 600, letterSpacing: "0.10em", textTransform: "uppercase", color: "var(--color-text-tertiary)", margin: 0, marginTop: 8 }}>
                      Due {assignment.due}
                    </p>
                  </div>
                ...
                >
                  <span style={{ fontFamily: font.body, fontSize: 13, lineHeight: "19px", color: "var(--color-text-secondary)", backgroundColor: "var(--color-bg-surface-2)", border: "1px solid var(--color-border-subtle)", borderRadius: 10, padding: "6px 12px" }}>
                ...
                <Markdown>{content.assignmentMd}</Markdown>
